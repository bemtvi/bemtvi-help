-- End-to-end :help: discover this plugin's own doc/ on the runtimepath, then open a
-- topic in the help split and land on its tag anchor. Run with `bemtvi --test-plugin`.
--
-- This exercises the real mechanism the unit specs stub out: rtp discovery via
-- btv.runtime_file (the neovim-identical "drop doc/ in a plugin" path) and the
-- btv.view-backed window.

local help = require("bemtvi-help")
local index = require("bemtvi-help.index")
local window = require("bemtvi-help.window")

-- The joined text of the help view buffer right now (or "" before it exists).
local function buf_text()
  local b = window.bufnr()
  if not b then
    return ""
  end
  return table.concat(btv.buf.lines(b, 0, -1, false), "\n")
end

-- Wait until the help buffer contains `needle`, returning the text.
local function wait_contains(t, needle)
  return t:wait_for(function()
    local txt = buf_text()
    return txt:find(needle, 1, true) and txt
  end)
end

btv.test.describe("bemtvi-help window", function()
  btv.test.before_each(function()
    window._reset()
    index.invalidate()
    help.setup()
  end)

  btv.test.after_each(function()
    window._reset()
  end)

  btv.test.it("discovers its own docs on the runtimepath (no tags file needed)", function()
    -- bemtvi-help ships no doc/tags; the index derives targets from doc/*.txt.
    local txts = btv.runtime_file("doc/*.txt", true)
    btv.test.expect(#txts >= 1).to_be_truthy()
    local idx = btv.await(index.build())
    btv.test.expect(idx["bemtvi-help"]).to_be_truthy()
    btv.test.expect(idx["bemtvi-help-usage"]).to_be_truthy()
  end)

  btv.test.it("indexes only real targets from the runtimepath docs", function()
    -- A doc that *writes about* help tags mentions `*targets*` / `*tag*` inline and
    -- shows `*ptr*`-style code in its examples. vim's scanner rejects both (not
    -- whitespace-delimited / inside a `>` example) and so must ours — otherwise this
    -- plugin's own doc squats on generic topics and `:help tag` lands on it.
    local idx = btv.await(index.build())
    for _, junk in ipairs({ "target", "targets", "tag", "structure", "my-topic" }) do
      btv.test.expect(idx[junk]).to_be_falsy()
    end
    -- the real tags are of course still there
    btv.test.expect(idx["bemtvi-help"]).to_be_truthy()
    btv.test.expect(idx["bemtvi-help-highlighting"]).to_be_truthy()
  end)

  btv.test.it("opens :help {topic} and lands on the tag anchor", function(t)
    -- Drive the real :help command: core defers it, the server sees the plugin's
    -- user command is registered, and runs it (the installed-plugin path).
    t:feed(":help bemtvi-help-usage<CR>")
    -- Wait on the resolved tag, not the rendered header casing.
    wait_contains(t, "bemtvi-help-usage")
    -- the cursor settles on the anchor line a tick later
    local line = t:wait_for(function()
      return window.line()
    end)
    local lines = btv.buf.lines(window.bufnr(), 0, -1, false)
    btv.test.expect(lines[line]).to_contain("bemtvi-help-usage")
  end)

  btv.test.it("prefix-resolves a partial topic", function(t)
    help.help("bemtvi-help-reg")
    btv.test.expect(wait_contains(t, "bemtvi-help-registering")).to_contain("bemtvi-help-registering")
  end)
end)
