#!/usr/bin/env bash
#
# Verify doc/nxvim-help.txt is up to date with the Markdown it is generated
# from. Regenerates it and fails if the result differs from what is committed — so a
# push can't ship a help file that no longer matches doc/nxvim-help.md. Wired
...
OUTPUT="doc/nxvim-help.txt" # must match OUTPUT in gen-vimdoc.sh
