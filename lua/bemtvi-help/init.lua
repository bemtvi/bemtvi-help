-- bemtvi-help — vim-style `:help` for bemtvi, built entirely on the native `btv.*`
...
-- (`:Plugins` calls `btv._add_rtp`), so `btv.runtime_file("doc/tags", true)` discovers
...
--   helptags.lua  `*target*` extraction and `doc/tags` generation (:BtvHelptags)
--   window.lua    show a resolved entry in the read-only help split (an btv.view)
...
--   picker.lua    the btv.picker source behind a bare `:help`
--   tagstack.lua  <C-]> / <CR> follow and <C-t> back
--   util.lua      the small glue the modules share (async runner, line splitting)
--
-- Quick start (init.lua): require("bemtvi-help").setup() — then `:help bemtvi-help`.
...
-- How many clashing tags `:BtvHelptags` names before it switches to "(+N more)".
...
local function open_first(candidates)
  run(function()
    ...
    btv.notify('E149: Sorry, no help for "' .. candidates[1] .. '"', 4)
  end)
end
...
-- :BtvHelptags [dir] — write a vim-style doc/tags from doc/*.txt. No argument (or
...
-- :BtvHelptags because bemtvi core owns (a stub of) :helptags.
function M.helptags(dir)
  run(function()
    ...
    for _, d in ipairs(dirs) do
      local res = btv.await(helptags.generate(d))
    ...
    btv.notify("bemtvi-help: wrote tags for " .. #dirs .. " dir(s), " .. total .. " tags")
  end)
end
...
-- caught in prose punctuation: `(btv.view)`, `|tag|`, a trailing full stop.
...
function M.help_cword()
  local candidates = M.cword_candidates(btv.expand("<cWORD>") or "")
  if #candidates == 0 then
    btv.notify("bemtvi-help: no word under the cursor", 3)
    return
  end
...
-- documented options honest: the auto-loader (plugin/bemtvi-help.lua) already ran
...
local function rebind(slot, lhs, rhs, desc)
  -- Withdraw the previous binding first — even at the SAME lhs, since `btv.keymap.set`
  -- appends rather than replaces, so re-running setup() would otherwise stack a
  -- duplicate mapping on every call.
  if bound[slot] then
    btv.keymap.del("n", bound[slot])
    bound[slot] = nil
  end
  if lhs then
    btv.keymap.set("n", lhs, rhs, { desc = desc })
    bound[slot] = lhs
  end
end

-- setup([opts]) — register the commands (once) and apply the per-call options. The
-- auto-loader (plugin/bemtvi-help.lua) calls setup() with no opts, and a user's later
...
function M.setup(opts)
  ...
  -- bemtvi core has no built-in :help, so this defines it; :h is the vim abbreviation.
  -- A core built-in (if one is ever added) takes precedence, so registering :h is safe.
  for _, name in ipairs({ "help", "h" }) do
    btv.user_command.create(name, function(a)
      M.help(a.args)
    end, { desc = "Open bemtvi help for {topic}" })
  end

  -- :BtvHelptags [dir|ALL] — (re)generate doc/tags. (:helptags is core-owned.)
  btv.user_command.create("BtvHelptags", function(a)
    M.helptags(a.args)
