<!-- DO NOT EDIT doc/bemtvi-help.txt BY HAND. It is generated from this file by
...
It works out of the box. The plugin auto-registers `:help` when it loads (`plugin/bemtvi-help.lua`
calls `setup()`), so once it is installed `:help {topic}` resolves against every installed
plugin's docs — this plugin's own included. Because bemtvi-help is optional, a plugin's `doc/`
is only viewable when the user has installed it; the docs ship harmlessly regardless.

<!-- Passed through to the vimdoc verbatim so `:help bemtvi-help` lands on this page
     (panvimdoc derives per-section tags but no bare project tag). -->
```vimdoc
                                              *bemtvi-help* *bemtvi-help-intro*
```
...
:BtvHelptags [dir] (Re)generate a vim-style `doc/tags` file — see Registering help.
```
...
Bare `:help` opens a fuzzy-finder over every known topic (`btv.picker`) with a live preview — the
...
installed plugin on the runtimepath (via `btv._add_rtp`), so `btv.runtime_file` finds every `doc/`.

A `tags` file is optional. If a `doc/` has one it is used (fast, and readable by vim);
otherwise bemtvi-help derives the `*targets*` straight from `doc/*.txt`, so help works with zero
...
`:BtvHelptags [dir]` writes such a file from `doc/*.txt`. With no argument (or `ALL`) it
regenerates every `doc/` directory on the runtimepath; with a directory argument it does just
that one. It is named `:BtvHelptags` because bemtvi core owns `:helptags`. Because the index reads
...
Lookup is exact-first, then the shortest prefix match: `:help bemtvi-help-u` resolves to
`bemtvi-help-usage`. An unmatched topic is a loud `E149`.
...
2. The token with surrounding punctuation trimmed, which catches a tag caught in prose
   punctuation: `(btv.view)`, `|tag|`, a trailing full stop.
...
false` could never take effect). Only maps bemtvi-help itself bound are ever removed.

```lua
require("bemtvi-help").setup({
...
```
btvHelpHeadline   Title              a section heading
btvHelpDelim      Comment            the ==== / ---- rules
btvHelpTag        Label              a *target*
btvHelpLink       Identifier         a |hot-link|
btvHelpCode       String             an inline `code` span, and code-block text
btvHelpCodeBlock  @markup.raw.block  a code block's full-width background
```
...
it. bemtvi has no `'conceal'`, so bemtvi-help rewrites those markers out of the displayed text
...
injected code block is anywhere else in bemtvi. A language with no grammar simply keeps the flat
code colour.

# Notes

bemtvi-help is built on the native `btv.*` surfaces only — `btv.view` (the read-only split), `btv.fs`
(promise reads), `btv.picker` (the topic finder), `btv.runtime_file` (runtimepath discovery),
`btv.buf.set_extmark` (the highlighting), `btv.treesitter.highlight` (code-block tokens), and
`btv.keymap` / `btv.user_command` / `btv.autocmd` (the bindings). It adds no core changes; it is a
worked example of a substantial editor feature living entirely as an `btv.*` plugin.
...
the same scan rather than each walking the runtimepath. `:BtvHelptags` invalidates it.
